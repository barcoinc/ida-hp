export { default as HeroSection } from './HeroSection';
export { default as WhySection } from './WhySection';
export { default as WhyDetailSection } from './WhyDetailSection';
export { default as WhatSection } from './WhatSection';
export { default as WhatDetailSection } from './WhatDetailSection';
export { default as HowServiceSection } from './HowServiceSection';
export { default as HowSection } from './HowSection';
export { default as FlowSection } from './FlowSection';
export { default as TargetSection } from './TargetSection';
export { default as ProfileSection } from './ProfileSection';
export { default as CTASection } from './CTASection';
export { default as ScrollSlideCurtain } from './ScrollSlideCurtain';
export { default as ScrollSlideWhy } from './ScrollSlideWhy';
export { default as ScrollSlideWhy1 } from './ScrollSlideWhy1';
export { default as ScrollSlideWhy2 } from './ScrollSlideWhy2';
export { default as ScrollSlideWhy3 } from './ScrollSlideWhy3';
export { default as ScrollSlideWhyFull } from './ScrollSlideWhyFull';
export { default as ScrollSlideWhat } from './ScrollSlideWhat';
export { default as ScrollSlideWhatDetail } from './ScrollSlideWhatDetail';
export { default as ScrollSlideService } from './ScrollSlideService';
export { default as ScrollSlideHowService } from './ScrollSlideHowService';
export { default as ScrollSlideHow } from './ScrollSlideHow';
